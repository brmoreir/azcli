#######################################################################################################################################
# Basic commands to create an EC2 instance with encryption enabled with CMK in AWS CLI.
#
...
# Support documentation here - https://awscli.amazonaws.com/v2/documentation/api/latest/reference/kms/create-key.html

aws kms create-key 
aws kms create-alias --alias-name "alias/my_symetric_cmk" --target-key-id "fb1246f4-9051-4843-81fd-621dc020f34f"

#ModifyEbsDefaultKmsKeyId operation changes the default KMS key for EBS encryption by default for your account in this Region
aws kms modifyEbsDefaultKmsKeyId
...
aws ec2 run-instances --image-id $amiId --count 1 --instance-type $ec2_size --key-name $kp_name --security-group-ids $sgId --subnet-id $subId
